...
(async () => {
  ...
  await click('#modal-btn-0'); await wait(async () => (await meta()).revision === 1, 'activation'); await wait(() => js("!document.body.hasAttribute('aria-busy') && !document.querySelector('#modal[open]')"), 'activation UI'); assert.equal((await meta()).codes, require('../forecast-core').topology(require('../server/demo-data').demo().tables).codes.length); checks.push('Excel upload, field mapping, build, explicit activation');
