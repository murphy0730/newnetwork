...
function createOpenAPI() {
  ...
  paths['/api/jobs/{id}/issues'] = { get: { operationId: 'download_import_issues', description: '下载所属任务的完整问题CSV，含原始来源与行内容。成功和失败任务均可下载，保留7天；table为空时下载全部。', parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }, { name: 'table', in: 'query', schema: { enum: ['forecast', 'adjust', 'bom', 'inventory', 'attributes', 'industry', 'mo', 'unknown'] } }], responses: { 200: { description: 'UTF-8 BOM CSV，逐行输出，不受页面预览100条限制', content: { 'text/csv': { schema: { type: 'string' } } } }, 404: response, 409: response } } };
