...
function toolsManifest() {
  return { instructions: '工具输出携带trace。回答时注明数据版本、预测版本、月份、分析范围、基线或推演；未知数据不能视为零。下层总缺口不得分别当成每个上层的分配缺口。导入与配置需要管理员，模拟需要planner权限；模型没有执行SQL、代码或发布基线的工具。', tools: [
    ['summarize_code', '\u751f\u6210\u7f16\u7801\u4f9b\u9700\u3001\u4e0b\u5c42\u98ce\u9669\u3001\u5468\u671f\u548c\u52a0\u5de5\u5730\u7684\u53ef\u6838\u9a8c\u6d1e\u5bdf\uff0c\u542b\u8bc1\u636e\u4e0e\u63a8\u6f14\u5efa\u8bae\u3002'], ['analyze_supply', '查看三口径供需匹配，可切换需求方查看下层总量风险。'], ['explain_code', '查看编码需求贡献、加工地、周期、累计覆盖及来源。'], ['inspect_network', '获取有界BOM子图和关键/风险路径。'], ['get_reports', '获取风险榜单、加工地集中度及逐月热力数据。'], ['simulate_forecast', '创建独立预测增减、产出损失或加工地延期情景，不修改基线。']
...
function createOpenAPI() {
  ...
  const reads = { '/api/insights': ['summarize_code', '\u7f16\u7801\u89c4\u5219\u6d1e\u5bdf\u4e0e\u8bc1\u636e'], '/api/meta': ['metadata', '数据规模、版本、加工地、配置、历史快照'], '/api/analysis': ['analyze_supply', '供需匹配分页查询'], '/api/nodes': ['explain_code', '编码详情及来源'], '/api/graph': ['inspect_network', '有界图谱'], '/api/reports': ['get_reports', '风险报告'], '/api/tables': ['read_table', '数据表分页查询'], '/api/jobs/{id}': ['get_job', '异步任务状态'], '/api/ai/tools': ['ai_tools', '模型工具定义'] };
