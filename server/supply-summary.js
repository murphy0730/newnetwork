...
function view(engine, month, mode, source) {
  ...
  const rows = [...byCode.values()].sort(Query.compare);
...
function decorate(engine, base, month, span, months, mode, source) {
  ...
  const monthly = months.map((m, i) => {
    const own = engine.row(base.code, m, 'direct', source);
    ...
    const complete = own.supply != null && demandKnown;
    const gap = !applicable ? 0 : complete ? demand - own.supply : null;
    return { month: m, raw: own.raw, add: own.add, remove: own.remove, supply: own.supply, demand, gap, inventory: own.inventory, coverageGap: !applicable ? 0 : gap != null && own.inventory != null ? gap - own.inventory : null, complete };
  });
  ...
  const risks = base.risks.filter(r => r !== '预测总量不足' && r !== '预测或产业数据不完整');
  if (!complete) risks.unshift('预测或产业数据不完整');
  if (gap > C.EPS) risks.unshift('预测总量不足');
  ...
  return { ...base, raw: sum('raw'), add: sum('add'), remove: sum('remove'), applicable, targets, targetCount: targets.length, supply, demand, gap, inventory, coverageGap: !applicable ? 0 : gap != null && inventory != null ? gap - inventory : null, firstShortage, complete, risks, monthly: span > 1 ? monthly : undefined };
}
module.exports = { relations, decorate, view };
